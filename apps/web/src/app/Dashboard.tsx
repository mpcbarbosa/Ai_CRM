...
const ACTIVITY_ICONS: Record<string, string> = {
  CALL: 'ð', EMAIL: 'ð§', MEETING: 'ð¤', NOTE: 'ð', TASK: 'â'
};
...
function ProbBadge({ value }: { value: string }) {
  ...
  const med = ['media', 'mÃ©dio', 'medio', 'medium'].includes(v);
      ...
      <div style={modalBox}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            ...
            <div style={{ color: '#64748b', fontSize: '13px' }}>
              {c.sector && <span style={{ marginRight: '12px' }}>ð {c.sector}</span>}
              {c.country && <span style={{ marginRight: '12px' }}>ð {c.country}</span>}
              {c.website && <a href={c.website} target="_blank" style={{ color: '#7c3aed' }}>ð {c.website}</a>}
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#64748b', fontSize: '24px', cursor: 'pointer' }}>â</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid #1e293b', marginBottom: '20px' }}>
          {(['overview', 'signals', 'activities', 'opportunities'] as const).map(t => (
            <button key={t} style={tabStyle(activeTab === t)} onClick={() => setActiveTab(t)}>
              {t === 'overview' ? 'VisÃ£o Geral' : t === 'signals' ? `Sinais (${c.signals?.length || 0})` : t === 'activities' ? `Atividades (${lead.activities?.length || 0})` : `Oportunidades (${lead.opportunities?.length || 0})`}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <div>
            ...
            {c.signals?.[0] && (
              <div>
                <div style={{ color: '#94a3b8', fontSize: '12px', fontWeight: 700, marginBottom: '8px', textTransform: 'uppercase' }}>Ãltimo Sinal</div>
                <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px' }}>
                  ...
                  {c.signals[0].sourceUrl && <a href={c.signals[0].sourceUrl} target="_blank" style={{ color: '#7c3aed', fontSize: '12px', display: 'block', marginTop: '10px' }}>ð Ver fonte</a>}
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {c.signals?.map((s: any) => (
              <div key={s.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '14px' }}>
                ...
                {s.sourceUrl && <a href={s.sourceUrl} target="_blank" style={{ color: '#7c3aed', fontSize: '11px', display: 'block', marginTop: '6px' }}>ð Fonte</a>}
              </div>
            ))}
          </div>
        )}
          ...
          <div>
            ...
            {showActivityForm && (
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                ...
                <input placeholder="TÃ­tulo da atividade *" value={newActivity.title}
              ...
              {lead.activities?.map((a: any) => (
                <div key={a.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '12px', display: 'flex', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#475569', fontSize: '11px' }}>{a.type} Â· {new Date(a.createdAt).toLocaleDateString('pt-PT')}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            {showOppForm && (
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                ...
                <input placeholder="Valor estimado (â¬)" value={newOpp.estimatedValue}
                  onChange={e => setNewOpp(o => ({ ...o, estimatedValue: e.target.value }))} style={inputStyle} type="number" />
                <input placeholder="ResponsÃ¡vel" value={newOpp.owner}
              ...
              {lead.opportunities?.map((o: any) => (
                <div key={o.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    ...
                    {o.estimatedValue && <span style={{ color: '#4ade80', fontWeight: 700 }}>â¬{Number(o.estimatedValue).toLocaleString('pt-PT')}</span>}
                  </div>
                  {o.owner && <div style={{ color: '#94a3b8', fontSize: '12px' }}>ð¤ {o.owner}</div>}
    ...
    <>
      ...
      {!loading && tab === 'pipeline' && (
        <>
          ...
          <table><thead><tr>
            ...
            <th>Ãltimo Trigger</th><th>Agente</th><th>Ãltima Atividade</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'clevel' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>PaÃ­s</th><th>Setor</th><th>Pessoa</th><th>Cargo</th><th>Impacto ERP</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
            {clevels.length === 0 ? <tr><td colSpan={8}><EmptyState msg="Nenhuma alteraÃ§Ã£o C-Level registada." /></td></tr>
              : clevels.map((s: any) => { const r = s.rawData || {};
                return <tr key={s.id} onClick={() => { const l = leads.find(l => l.companyId === s.companyId); if (l) router.push('/leads/' + l.id); }} style={{ cursor: 'pointer' }}>
                  ...
                  <td>{s.sourceUrl ? <a href={s.sourceUrl} target="_blank" style={{ color: '#7c3aed' }} onClick={e => e.stopPropagation()}>ð</a> : '-'}</td>
      ...
      {!loading && tab === 'rfp' && (
        <>
          ...
          <table><thead><tr>
            <th>Entidade</th><th>PaÃ­s</th><th>DescriÃ§Ã£o</th><th>Valor Estimado</th><th>PertinÃªncia ERP</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'expansion' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>PaÃ­s</th><th>Setor</th><th>Tipo ExpansÃ£o</th><th>Impacto ERP</th><th>Probabilidade</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'scoring' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>PaÃ­s</th><th>Setor</th><th>Score Final</th><th>Trigger</th><th>Probabilidade</th><th>Resumo</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'sectors' && (
        <>
          ...
          <table><thead><tr>
            <th>Setor</th><th>Crescimento</th><th>Investimento</th><th>Maturidade Tech</th><th>Drivers</th><th>Prob. ERP</th><th>NotÃ­cias</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
            {sectors.length === 0 ? <tr><td colSpan={9}><EmptyState msg="Nenhuma anÃ¡lise setorial registada." /></td></tr>
              : sectors.map((s: any) => { const r = s.rawData || {};
                return <tr key={s.id}>
                  ...
                  <td>{r.fonte_principal ? <a href={r.fonte_principal} target="_blank" style={{ color: '#7c3aed' }}>ð</a> : '-'}</td>
                  <td style={{ color: '#64748b', fontSize: '12px' }}>{new Date(s.createdAt).toLocaleDateString('pt-PT')}</td>
                </tr>; })}
          </tbody></table>
        </>
