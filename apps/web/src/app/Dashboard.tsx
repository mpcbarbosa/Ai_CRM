...
const ACTIVITY_ICONS: Record<string, string> = {
  CALL: 'Ã°ÂÂÂ', EMAIL: 'Ã°ÂÂÂ§', MEETING: 'Ã°ÂÂ¤Â', NOTE: 'Ã°ÂÂÂ', TASK: 'Ã¢ÂÂ'
};
...
function ProbBadge({ value }: { value: string }) {
  ...
  const med = ['media', 'mÃÂ©dio', 'medio', 'medium'].includes(v);
      ...
      <div style={modalBox}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            ...
            <div style={{ color: '#64748b', fontSize: '13px' }}>
              {c.sector && <span style={{ marginRight: '12px' }}>Ã°ÂÂÂ {c.sector}</span>}
              {c.country && <span style={{ marginRight: '12px' }}>Ã°ÂÂÂ {c.country}</span>}
              {c.website && <a href={c.website} target="_blank" style={{ color: '#7c3aed' }}>Ã°ÂÂÂ {c.website}</a>}
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#64748b', fontSize: '24px', cursor: 'pointer' }}>Ã¢ÂÂ</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid #1e293b', marginBottom: '20px' }}>
          {(['overview', 'signals', 'activities', 'opportunities'] as const).map(t => (
            <button key={t} style={tabStyle(activeTab === t)} onClick={() => setActiveTab(t)}>
              {t === 'overview' ? 'VisÃÂ£o Geral' : t === 'signals' ? `Sinais (${c.signals?.length || 0})` : t === 'activities' ? `Atividades (${lead.activities?.length || 0})` : `Oportunidades (${lead.opportunities?.length || 0})`}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <div>
            ...
            {c.signals?.[0] && (
              <div>
                <div style={{ color: '#94a3b8', fontSize: '12px', fontWeight: 700, marginBottom: '8px', textTransform: 'uppercase' }}>ÃÂltimo Sinal</div>
                <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px' }}>
                  ...
                  {c.signals[0].sourceUrl && <a href={c.signals[0].sourceUrl} target="_blank" style={{ color: '#7c3aed', fontSize: '12px', display: 'block', marginTop: '10px' }}>Ã°ÂÂÂ Ver fonte</a>}
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {c.signals?.map((s: any) => (
              <div key={s.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '14px' }}>
                ...
                {s.sourceUrl && <a href={s.sourceUrl} target="_blank" style={{ color: '#7c3aed', fontSize: '11px', display: 'block', marginTop: '6px' }}>Ã°ÂÂÂ Fonte</a>}
              </div>
            ))}
          </div>
        )}
          ...
          <div>
            ...
            {showActivityForm && (
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                ...
                <input placeholder="TÃÂ­tulo da atividade *" value={newActivity.title}
              ...
              {lead.activities?.map((a: any) => (
                <div key={a.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '12px', display: 'flex', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#475569', fontSize: '11px' }}>{a.type} ÃÂ· {new Date(a.createdAt).toLocaleDateString('pt-PT')}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            {showOppForm && (
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                ...
                <input placeholder="Valor estimado (Ã¢ÂÂ¬)" value={newOpp.estimatedValue}
                  onChange={e => setNewOpp(o => ({ ...o, estimatedValue: e.target.value }))} style={inputStyle} type="number" />
                <input placeholder="ResponsÃÂ¡vel" value={newOpp.owner}
              ...
              {lead.opportunities?.map((o: any) => (
                <div key={o.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    ...
                    {o.estimatedValue && <span style={{ color: '#4ade80', fontWeight: 700 }}>Ã¢ÂÂ¬{Number(o.estimatedValue).toLocaleString('pt-PT')}</span>}
                  </div>
                  {o.owner && <div style={{ color: '#94a3b8', fontSize: '12px' }}>Ã°ÂÂÂ¤ {o.owner}</div>}
...
export default function Dashboard() {
  ...
  const tabs = [
    ...
    { id: 'expansion', label: 'ExpansÃÂ£o', count: expansions.length },
    ...
    <>
      ...
      <div className="kpi-grid">
        {[
          ...
          { label: 'Pipeline Total', value: 'Ã¢ÂÂ¬' + Number(stats.pipeline || 0).toLocaleString('pt-PT'), sub: 'valor estimado', cls: 'orange' },
        ].map(k => (
      ...
      {!loading && tab === 'pipeline' && (
        <>
          ...
          <table><thead><tr>
            ...
            <th>ÃÂltimo Trigger</th><th>Agente</th><th>ÃÂltima Atividade</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'clevel' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>PaÃÂ­s</th><th>Setor</th><th>Pessoa</th><th>Cargo</th><th>Impacto ERP</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
            {clevels.length === 0 ? <tr><td colSpan={8}><EmptyState msg="Nenhuma alteraÃÂ§ÃÂ£o C-Level registada." /></td></tr>
              : clevels.map((s: any) => { const r = s.rawData || {};
                return <tr key={s.id} onClick={() => { const l = leads.find(l => l.companyId === s.companyId); if (l) router.push('/leads/' + l.id); }} style={{ cursor: 'pointer' }}>
                  ...
                  <td>{s.sourceUrl ? <a href={s.sourceUrl} target="_blank" style={{ color: '#7c3aed' }} onClick={e => e.stopPropagation()}>Ã°ÂÂÂ</a> : '-'}</td>
      ...
      {!loading && tab === 'rfp' && (
        <>
          <div className="section-title">RFP / Concursos PÃÂºblicos</div>
          <table><thead><tr>
            <th>Entidade</th><th>PaÃÂ­s</th><th>DescriÃÂ§ÃÂ£o</th><th>Valor Estimado</th><th>PertinÃÂªncia ERP</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'expansion' && (
        <>
          <div className="section-title">ExpansÃÂ£o de Empresas</div>
          <table><thead><tr>
            <th>Empresa</th><th>PaÃÂ­s</th><th>Setor</th><th>Tipo ExpansÃÂ£o</th><th>Impacto ERP</th><th>Probabilidade</th><th>Data</th>
          </tr></thead><tbody>
            {expansions.length === 0 ? <tr><td colSpan={7}><EmptyState msg="Nenhuma expansÃÂ£o registada." /></td></tr>
      ...
      {!loading && tab === 'scoring' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>PaÃÂ­s</th><th>Setor</th><th>Score Final</th><th>Trigger</th><th>Probabilidade</th><th>Resumo</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'sectors' && (
        <>
          <div className="section-title">AnÃÂ¡lise Setorial Ã¢ÂÂ Portugal</div>
          <table><thead><tr>
            <th>Setor</th><th>Crescimento</th><th>Investimento</th><th>Maturidade Tech</th><th>Drivers</th><th>Prob. ERP</th><th>NotÃÂ­cias</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
            {sectors.length === 0 ? <tr><td colSpan={9}><EmptyState msg="Nenhuma anÃÂ¡lise setorial registada." /></td></tr>
              : sectors.map((s: any) => { const r = s.rawData || {};
                return <tr key={s.id}>
                  ...
                  <td>{r.fonte_principal ? <a href={r.fonte_principal} target="_blank" style={{ color: '#7c3aed' }}>Ã°ÂÂÂ</a> : '-'}</td>
                  <td style={{ color: '#64748b', fontSize: '12px' }}>{new Date(s.createdAt).toLocaleDateString('pt-PT')}</td>
                </tr>; })}
          </tbody></table>
        </>
