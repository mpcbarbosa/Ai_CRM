...
function ProbBadge({ value }: { value: string }) {
  ...
  const med = ['media', 'médio', 'medio', 'medium'].includes(v);
      ...
      <div style={modalBox}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            ...
            <div style={{ color: '#64748b', fontSize: '13px' }}>
              {c.sector && <span style={{ marginRight: '12px' }}>📂 {c.sector}</span>}
              {c.country && <span style={{ marginRight: '12px' }}>🌍 {c.country}</span>}
              {c.website && <a href={c.website} target="_blank" style={{ color: '#7c3aed' }}>🔗 {c.website}</a>}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid #1e293b', marginBottom: '20px' }}>
          {(['overview', 'signals', 'activities', 'opportunities'] as const).map(t => (
            <button key={t} style={tabStyle(activeTab === t)} onClick={() => setActiveTab(t)}>
              {t === 'overview' ? 'Visão Geral' : t === 'signals' ? `Sinais (${c.signals?.length || 0})` : t === 'activities' ? `Atividades (${lead.activities?.length || 0})` : `Oportunidades (${lead.opportunities?.length || 0})`}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <div>
            ...
            {c.signals?.[0] && (
              <div>
                <div style={{ color: '#94a3b8', fontSize: '12px', fontWeight: 700, marginBottom: '8px', textTransform: 'uppercase' }}>Último Sinal</div>
                <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px' }}>
                  ...
                  {c.signals[0].sourceUrl && <a href={c.signals[0].sourceUrl} target="_blank" style={{ color: '#7c3aed', fontSize: '12px', display: 'block', marginTop: '10px' }}>🔗 Ver fonte</a>}
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {c.signals?.map((s: any) => (
              <div key={s.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '14px' }}>
                ...
                {s.sourceUrl && <a href={s.sourceUrl} target="_blank" style={{ color: '#7c3aed', fontSize: '11px', display: 'block', marginTop: '6px' }}>🔗 Fonte</a>}
              </div>
            ))}
          </div>
        )}
          ...
          <div>
            ...
            {showActivityForm && (
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                ...
                <input placeholder="Título da atividade *" value={newActivity.title}
              ...
              {lead.activities?.map((a: any) => (
                <div key={a.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '12px', display: 'flex', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#475569', fontSize: '11px' }}>{a.type} · {new Date(a.createdAt).toLocaleDateString('pt-PT')}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            {showOppForm && (
              <div style={{ background: '#0f172a', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                ...
                <input placeholder="Valor estimado (€)" value={newOpp.estimatedValue}
                  onChange={e => setNewOpp(o => ({ ...o, estimatedValue: e.target.value }))} style={inputStyle} type="number" />
                <input placeholder="Responsável" value={newOpp.owner}
              ...
              {lead.opportunities?.map((o: any) => (
                <div key={o.id} style={{ background: '#0f172a', borderRadius: '8px', padding: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    ...
                    {o.estimatedValue && <span style={{ color: '#4ade80', fontWeight: 700 }}>€{Number(o.estimatedValue).toLocaleString('pt-PT')}</span>}
                  </div>
                  {o.owner && <div style={{ color: '#94a3b8', fontSize: '12px' }}>👤 {o.owner}</div>}
    ...
    <>
      ...
      {!loading && tab === 'clevel' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>País</th><th>Setor</th><th>Pessoa</th><th>Cargo</th><th>Impacto ERP</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
            {clevels.length === 0 ? <tr><td colSpan={8}><EmptyState msg="Nenhuma alteração C-Level registada." /></td></tr>
              : clevels.map((s: any) => { const r = s.rawData || {};
                return <tr key={s.id} onClick={() => { const l = leads.find(l => l.companyId === s.companyId); if (l) setSelectedLeadId(l.id); }} style={{ cursor: 'pointer' }}>
                  ...
                  <td>{s.sourceUrl ? <a href={s.sourceUrl} target="_blank" style={{ color: '#7c3aed' }} onClick={e => e.stopPropagation()}>🔗</a> : '-'}</td>
      ...
      {!loading && tab === 'rfp' && (
        <>
          ...
          <table><thead><tr>
            <th>Entidade</th><th>País</th><th>Descrição</th><th>Valor Estimado</th><th>Pertinência ERP</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'expansion' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>País</th><th>Setor</th><th>Tipo Expansão</th><th>Impacto ERP</th><th>Probabilidade</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'scoring' && (
        <>
          ...
          <table><thead><tr>
            <th>Empresa</th><th>País</th><th>Setor</th><th>Score Final</th><th>Trigger</th><th>Probabilidade</th><th>Resumo</th><th>Data</th>
          </tr></thead><tbody>
      ...
      {!loading && tab === 'sectors' && (
        <>
          ...
          <table><thead><tr>
            <th>Setor</th><th>Crescimento</th><th>Investimento</th><th>Maturidade Tech</th><th>Drivers</th><th>Prob. ERP</th><th>Notícias</th><th>Fonte</th><th>Data</th>
          </tr></thead><tbody>
